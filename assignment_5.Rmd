---
title: "ESM 206, Assignment 5"
author: "Andrew Salvador & Benson Truong"
date: "11/29/2019"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE,
                      message = FALSE,
                      warning = FALSE)
```

NOTE: We are not capitalizing "old growth" and "clear cut" throughout the document, unless using the abbreviation (OG) & (CC)



```{r}
# Attach necessary packages

library(tidyverse)
library(janitor)
library(effsize)
library(kableExtra)
library(ggpubr)
library(ggbeeswarm)
library(broom)
library(devtools)
library(car)
```

```{r}
# Read in data

initial_data <- read_csv("mack_creek_vertebrates.csv")

# Clean up data and filter for only Pacific Giant Salamanders (species code "DITE") 
# pgs = pacific giant salamander

pgs_clean <- initial_data %>% 
  filter(SPECIES == "DITE") %>% 
  clean_names() 
  

```

## Introduction

## Data & Methods

## Results {.tabset .tabset-fade}

### Results A

#### Pacific giant salamander abundance
Salamander count in clear cut (CC) and old growth (OG) sections of Mack Creek from 1993 to 2017. Trends are fairly similar in both sections, with salamander abundance increasing sharply in 2002, decreasing rapidly around the year 2012, and then increasing again beginning in 2015. For all but two years from 1993 to 2014 salamander count in the old growth section was greater than in the clear cut section, with the two exceptions being 1994 and 2012, where salamander count in the clear cut section slightly exceeded salamander observations in the old growth section. Salamander count in the clear cut section was greater than in the old growth section for all three years from 2015 to 2017. 
```{r}

# Filter by section and year
pgs_count <- pgs_clean %>% 
  count(section, year) %>% 
  mutate(year = as.character(year))

# Rename the years using abbreviations 
pgs_count$year <- gsub("1993", "'93", pgs_count$year)
pgs_count$year <- gsub("1994", "'94", pgs_count$year)
pgs_count$year <- gsub("1995", "'95", pgs_count$year)
pgs_count$year <- gsub("1996", "'96", pgs_count$year)
pgs_count$year <- gsub("1997", "'97", pgs_count$year)
pgs_count$year <- gsub("1998", "'98", pgs_count$year)
pgs_count$year <- gsub("1999", "'99", pgs_count$year)
pgs_count$year <- gsub("2000", "'00", pgs_count$year)
pgs_count$year <- gsub("2001", "'01", pgs_count$year)
pgs_count$year <- gsub("2002", "'02", pgs_count$year)
pgs_count$year <- gsub("2003", "'03", pgs_count$year)
pgs_count$year <- gsub("2004", "'04", pgs_count$year)
pgs_count$year <- gsub("2005", "'05", pgs_count$year)
pgs_count$year <- gsub("2006", "'06", pgs_count$year)
pgs_count$year <- gsub("2007", "'07", pgs_count$year)
pgs_count$year <- gsub("2008", "'08", pgs_count$year)
pgs_count$year <- gsub("2009", "'09", pgs_count$year)
pgs_count$year <- gsub("2010", "'10", pgs_count$year)
pgs_count$year <- gsub("2011", "'11", pgs_count$year)
pgs_count$year <- gsub("2012", "'12", pgs_count$year)
pgs_count$year <- gsub("2013", "'13", pgs_count$year)
pgs_count$year <- gsub("2014", "'14", pgs_count$year)
pgs_count$year <- gsub("2015", "'15", pgs_count$year)
pgs_count$year <- gsub("2016", "'16", pgs_count$year)
pgs_count$year <- gsub("2017", "'17", pgs_count$year)
pgs_count$year <- gsub("2018", "'18", pgs_count$year)



# Graph it, setting tick marks every two years so the axis labels don't overlap
ggplot(pgs_count, aes(x = year,
                      y = n)) +
  geom_col(aes(color = section,
               fill = section,
               width = .7,
               alpha = .8),
               show.legend = FALSE) +
  scale_x_discrete(limits = c("'93", "'94", "'95", "'96", "'97", "'98", "'99", "'00", "'01", "'02", "'03", "'04", "'05", "'06", "'07", "'08", "'09", "'10", "'11", "'12", "'13", "'14", "'15", "'16", "'17"),
                   breaks = c("'93", "'95", "'97", "'99", "'01", "'03", "'05", "'07", "'09", "'11", "'13", "'15", "'17")) +
  scale_y_continuous(limits = c(0, 400), expand = c(0,0)) +
  facet_wrap(~section) +
  labs(x = "Year",
       y = "Salamander Count") 


```
***Figure 1.*** *Sample salamander counts from years 1993 to 2017 for clear cut (CC) and old growth (OG) sections of Mack Creek. Clear cut section shown in red while old growth section shown in blue. Data: Aquatic Vertebrate Population Study in Mack Creek, Andrews Experimental Forest, 1987 to present.*



```{r}

# A different visualization for salamander count
ggplot(pgs_count, aes(x = year,
                      y = n)) +
  geom_point(aes(color = section),
             size = 3,
             show.legend = FALSE) +
  scale_x_discrete(limits = c("'93", "'94", "'95", "'96", "'97", "'98", "'99", "'00", "'01", "'02", "'03", "'04", "'05", "'06", "'07", "'08", "'09", "'10", "'11", "'12", "'13", "'14", "'15", "'16", "'17"),
  breaks = c("'93", "'95", "'97", "'99", "'01", "'03", "'05", "'07", "'09", "'11", "'13", "'15", "'17")) +
  scale_y_continuous(limits = c(0, 400), expand = c(0,0)) +
  geom_segment(aes(x = year,
               y = 0,
               xend = year,
               yend = n,
               color = section),
               show.legend = FALSE) +
  facet_wrap(~section) +
  labs(x = "Year",
       y = "Salamander Count")

```



### Results B

***Table 1.*** *Counts and proportions of salamanders observed in different channel classifications (cascades, pool, side-channel) within two different sections (clear cut & old growth) in Mack Creek in 2017. Table excludes salamanders observed in isolated pools (IP) disconnected from the channel. Data: Aquatic Vertebrate Population Study in Mack Creek, Andrews Experimental Forest, 1987 to present.*
```{r}
# Count and proportions of salamanders in different channel classifications (pool (P) cascade (C), side-channel (SC)). Exclude Isolated Pools (IP)

pgs_class <- pgs_clean %>% 
  filter(year == "2017") %>% 
  count(section, unittype) %>% 
  filter(unittype %in% c("C", "P", "SC"))

# Use pivot wider to make a contingency table so the column headers are the channel classifications 

pgs_class_table <- pgs_class %>% 
  pivot_wider(names_from = unittype,
              values_from = n) %>% 
  rename("Cascades" = C, "Pool" = P, "Side-Channel" = SC, "Section" = section)

# Rename CC and OG 
pgs_class_table$Section <- gsub("CC", "Clear Cut", pgs_class_table$Section)
pgs_class_table$Section <- gsub("OG", "Old Growth", pgs_class_table$Section)

# use adorn funciton to show percentage and actual values
pgs_class_prop <- pgs_class_table %>% 
  janitor::adorn_percentages(denominator = "row") %>% 
  adorn_pct_formatting(digits = 2) %>% 
  adorn_ns(position = "front")

#Now convert to a kableExtra table

kable(pgs_class_prop) %>% 
  kable_styling()


```




### Results C

```{r}
# Now do a chi square test to see if section has any affect on channel classification
# We only want to use the counts so we change the section column into the rownames
contingency_table_c <- pgs_class_table %>% 
  column_to_rownames('Section')

# Now run the chi-square test 
channelclass_chi <- chisq.test(contingency_table_c)

```
#### Forest condition impact on Pacific giant salamander location in 2017
Forest condition (clear cut or old growth) did not have a significant effect on salamander location within the channel (cascade, side-channel, pool) (($\chi$^2^(`r channelclass_chi$parameter`)) = `r channelclass_chi$statistic`), *p* > 0.05). For salamander observations in the clear cut section n = 368, for observations in the old growth section n = 320. The greatest proportion of salamanders observed was in the cascades (67.12% clear cut, 62.81% old growth), while the smallest proportion of salamanders observed was in the pools (8.42% clear cut, 14.06% old growth). Actual counts for clear cut areas was 247 in cascades, 31 in pools, and 90 in side-channels, and for old growth forest, 201 in cascades, 45 in pools, and 74 in side-channels (Table 1).





### Results D
```{r}
# Testing the mean weights for PGS in 2017 for OG and CC sections
# For Old Growth
pgs_og <- pgs_clean %>% 
  filter(year == "2017",
         section == "OG") %>% 
  pull(weight)

# For Clear Cut
pgs_cc <- pgs_clean %>% 
  filter(year == "2017",
         section == "CC") %>% 
  pull(weight)


# Just making some vectors that show the mean of clear cut and og weights in 2017
cc_weight_2017 <- pgs_clean %>% 
  filter(year == "2017", section == "CC") 
 
mean_cc_weight_2017 <- mean(cc_weight_2017$weight, na.rm = TRUE)

og_weight_2017 <- pgs_clean %>% 
  filter(year == "2017", section == "OG")

mean_og_weight_2017 <- mean(og_weight_2017$weight, na.rm = TRUE)

og_cc_ttest <- t.test(pgs_og, pgs_cc)

# P-value of 0.09599, retain null hypothesis
# There is no significant difference between in mean weights for Pacific giant salamanders observed in the two forest sections in 2017. 
```
#### Comparing Pacific giant salamander weight in 2017
Mean salamander weight in clear cut and old growth sections did not differ significantly in 2017 (t(`r round(og_cc_ttest$parameter, 2)`) = `r round(og_cc_ttest$statistic, 2)`, *p* > 0.05). Mean salamander weight in the clear cut section was 18.11% greater than in the old growth section, with an actual difference of 1.19 grams. For salamanders observed in the clear cut section mean = 7.78g, n = 368, while for salamanders observed in the old growth section, mean = 6.58g, n = 320. 









### Results E

```{r}
# Compare weights of PGS in pools, cascades and side-channels of Mack Creek in 2017
pgs_2017_graph <- pgs_clean %>% 
  filter(year == "2017",
         unittype %in% c("P", "C", "SC"))

# Visually explore salamander weights to check for normality

# First a histogram

ggplot(pgs_2017_graph, aes(x = weight)) +
  geom_histogram(aes(color = unittype,
                     fill = unittype)) +
  facet_wrap(~unittype)

# Now a quantile-quantile plot

ggplot(pgs_2017_graph, aes(sample = weight)) +
  geom_qq() +
  facet_wrap(~unittype)

# All sample sizes > 30 so that is good for parametric test except....
# Both the histogram and the qq plot do not look really that normal....maybe that has to do with the third part of results 3? 


```


```{r}

# Check the variance 
levene_test <- leveneTest(weight ~ unittype, data = pgs_2017_graph)

#Ho: there is no significant variance of weight across the three different unitttypes (cascade, pool, side-channel)
# P value is .09 so cannot reject null. 
# We can thus assume homogeneity across groups and perform a one-way ANOVA (assumes equal variance across groups)

```

```{r}

# Graph weights across unittypes 
# Idk which graph looks the best. maybe jitter or histogram? 

# Summary statistic table to make error bars for graph
pgs_summary <- pgs_2017_graph %>% 
  group_by(unittype) %>% 
  summarize(
    mean_weight = mean(weight, na.rm = TRUE),
    sd_weight = sd(weight, na.rm = TRUE),
    sample_size = n(),
    se_weight = sd(weight, na.rm = TRUE)/sqrt(n()),
    var_weight = var(weight, na.rm = TRUE)
  )


# Histogram
ggplot(data = pgs_2017_graph, aes(x = weight))+
  geom_histogram(aes(fill=unittype), na.rm = TRUE)+
  facet_wrap(~unittype)

# Violin
ggplot(data = pgs_2017_graph, aes(x = unittype, y = weight))+
  geom_violin(aes(color=unittype), na.rm = TRUE)+
  facet_wrap(~unittype)

# Jitter
ggplot() +
  geom_jitter(data = pgs_2017_graph, 
                aes(x = unittype,
                    y = weight),
                size = 3, 
                alpha = 0.6,
                pch = 1,
                color = "gray50") +
  scale_x_discrete(labels = c( "Cascade", "Pool", "Side-Channel")) +
  geom_point(data = pgs_summary,
             aes(x = unittype,
                 y = mean_weight),
             color = "red") + 
  geom_errorbar(data = pgs_summary,
                aes(x = unittype,
                    ymin = mean_weight - sd_weight,
                    ymax = mean_weight + sd_weight),
                width = 0.1) +
  theme_light() +
  labs(x = "Unit Type",
       y = "Weight (g)")

               
# Beeswarm
ggplot() +
  geom_beeswarm(data = pgs_2017_graph, 
                aes(x = unittype,
                    y = weight),
                size = 3, 
                alpha = 0.6,
                pch = 16,
                color = "gray60") +#  Now i'm going to change the labels on the x axis tick marks
  scale_x_discrete(labels = c( "Cascade", "Pool", "Side-Channel")) +# now i'm going to indicate the mean
  scale_y_continuous(limits = c(-5,100), expand = c(0,0)) +
  geom_point(data = pgs_summary, 
             aes(x = unittype,
                 y = mean_weight),
             color = "red") + # Now plotting data spread (errorbar), 1 standard deviation up and down from mean (standard error and 95% coonfidence interval are also very common)
  geom_errorbar(data = pgs_summary,
                aes(x = unittype,
                    ymin = mean_weight - sd_weight,
                    ymax = mean_weight + sd_weight),
                width = 0.1) +
  theme_light() +
  labs(x = "Unit Type", 
       y = "Weight (g)")
  

```
**Figure 2.** *Salamander weight (g) for Cascade (n = 448), Side-Channel (n = 164), and Pool (n = 76) salamanders in Mack Creek, Oregon. Means indicated by red dots; error bars indicate $\pm$ 1 standard deviation. Data: Aquatic Vertebrate Population Study in Mack Creek, Andrews Experimental Forest, 1987 to present.*

```{r}

# Counting unit types shown for reproducibility 

unittype_count_2017 <- pgs_clean %>% 
  filter(year == "2017") %>% 
  count(unittype)
  

```

#### One-way ANOVA and Tukey's test
```{r}

# ANOVA Test
pgs_aov <- aov(weight~unittype, data = pgs_2017_graph)
pgs_aov_summary <- summary(pgs_aov)

# There is a significant difference in mean weights between cascade, side-channel, and pool salamanders in Mack Creek (F(2, 684) = 4.216, p < .05). IS THERE A WAY TO INLINE REFERENCE THIS?? I just hard coded it below...couldn't find anything in the lab to inline reference.

```
One-way ANOVA shows that there is a significant difference in mean weights between cascade, side-channel, and pool salamanders in Mack Creek (F(2, 684) = 4.216, *p* < 0.05). 
```{r}

# They are not all the same weight, so which ones are different?

tukeys_test <- TukeyHSD(pgs_aov)

# p adj shows the hypothesis test. Only the p value for side channel compared to pool is < 0.05 so those are the two that are significantly different. 
```
A post-hoc Tukey's HSD shows a significant difference in mean weight between side-channel and pool salamanders (p < 0.05). Mean weight of salamanders found in pools was 64% greater than that of side-channel salamanders, with an actual difference of 3.61g, where for side-channel salamanders, mean = 5.68g, n = 164, and for pool salamanders, mean = 9.29g, n = 76 (Figure 2). Pool and cascade salamanders, and side-channel and cascade salamanders did not differ significantly in mean weight (p > 0.05). For cascade salamanders, mean = 7.52g, n = 448. Mean weight of pool salamanders was 23% greater than cascade salamanders, while mean weight of cascade salamanders was 32% greater than side channel salamanders. Standard deviations for all groups are fairly high (cascade SD = 9.03, pool SD = 13.62, side-channel SD = 8.27), but can be accounted for due to several outliers in each groups, influencing the data spread (Figure 2).

*Should we keep what I wrote about standard deviation or do you think we should just not talk about it? Also is what I wrote true lol*

## Summary

## References 
Gregory S. V. 2016. Aquatic Vertebrate Population Study in Mack Creek, Andrews Experimental Forest, 1987 to present. Environmental Data Initiative. https://doi.org/10.6073/pasta/5de64af9c11579266ef20da2ff32f702. Dataset accessed 11/30/2019.



Things I wasn't sure about:

1. Results E the First part: instructions say "indicate values for the mean, and standard deviation, standard error *or* a 95% confidence interval". 
- We have the mean, and standard deviation...I'm not sure we have standard error or a 95% confidence interval. 
-Do we need the SE or the 95% confidence interval? The way she worded it is weird.

2. For Third part of Results E. 
- Maybe has to do with the fact that the histogram and qq plot do *not* really look normal?

3. For the beeswarm plot: there are a couple outliers for pool side channel that have a weight value of  like 85 and 88 so I made the y-scale to 100...

4. Is the reference formatting correctly? I just copied what was in the assignment. Instructions state formatting matters

5. I think (haven't read through whole thing yet) the report might be partially in present tense (does have an effect) and partially in past (did have an effect). Should probably pick one and stiock with it.

6. For results E: I think the beeswarm plot represents the data the best but let me know what you think. We also probably need to hide the exploration visualizations checking for normality (qq and histogram) in the final report? I think?

7. For results A I could go with either the bar graph or lollipop but would probably pick the bar graph. Either one is fine.


Benson Response:
 1. Yeah not sure, we can ask her about this but I think adding the mean and SD in the graph is good enough. If anything, we can add the SE values in the figure captions.
 
 4. It looks good, just need to italicize the article name
 
 5. Yeah lets go with present tense
 
 6. I like the jitter plot, the beeswarm looks like a sad christmas tree LOL. I think either is fine. Yeah, we should hide the qq and histogram in final report.
 
 7. Yeah I like the bar graph better.
 







