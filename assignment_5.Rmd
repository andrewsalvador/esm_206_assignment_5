...
ggplot(data = pgs_2017_graph, aes(x = unittype, y = weight))+
  geom_jitter(aes(color=unittype), na.rm = TRUE,
              width = .2,
              alpha = .4)+
  scale_x_discrete(labels = c("Cascades", "Pool", "Side Channel"))+
  geom_point(data = pgs_summary, aes(x = unittype, y = mean_weight),
             color = "black",
             size = 3)+
  geom_errorbar(data = pgs_summary, #error when trying to add error bars
                  aes(x = unittype,
